...
def show_gerenciar_cliente(psicologo_responsavel):
    ...
    # --- Notas Fiscais com 2 tabs internas (Envio | Documentos) ---
    with tabs[3]:
        st.subheader("📁 Notas Fiscais")

        tipo = "Notas Fiscais"
        bucket_name = os.getenv("GCS_BUCKET_NAME")
        pasta_cliente = f"{cliente_nome}"  # prefixo base no GCS

        tab_envio, tab_lista = st.tabs(["📥 Enviar", "📄 Documentos"])

        # ===== TAB 1: Envio =====
        with tab_envio:
            st.markdown("### 📥 Enviar documento de **Notas Fiscais**")
        ...
        # ===== TAB 2: Lista =====
    with tab_lista:
        st.markdown(f"### 📖 Documentos de **{tipo}**")
        ...
        for blob in arquivos or []:
            ...
            try:
                ...
                if tamanho_bytes is None:
                    tamanho_bytes = len(blob.download_as_bytes())  # fallback
                tamanho_leg = f"{tamanho_bytes//1024} KB"
            except Exception:
                tamanho_leg = "-"
        ...
        if total == 0:
            ...
            # paginação (controle fica aqui, abaixo do título da lista)
            ...
            for i, it in enumerate(visiveis):
                ...
                with st.container():
                    ...
                    with c1:
                        st.markdown(
                            f"<div class='file-card file-row' style='color:#111;'>📕 <b>{it['nome_amigavel']}</b></div>",
                            unsafe_allow_html=True
                        )
                    with c2:
